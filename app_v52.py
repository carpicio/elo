...
st.title("🛡️ Calcolatore Strategico (Final Fix)")
st.markdown("---")
...
def calculate_row(row, base_hfa, use_dynamic):
    ...
    try:
        ...
        if use_dynamic:
            r_h_home = row.get('rank_h_home') # Mapped from Place 1a
            r_a_away = row.get('rank_a_away') # Mapped from Place 2d
